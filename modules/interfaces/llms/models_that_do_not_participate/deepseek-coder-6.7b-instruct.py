...
class DeepSeekCoder67bInstruct(LLM):
    ...
    def initialize(self, server_is_listening=False) -> int:
        command = f"""ssh -tt tcappendij@129.152.15.84 'cd data/volume_2/Greencode_framework/ && source venv/bin/activate && python3 deepseek-coder-6.7b-instruct/server.py --port {self.port} --host {self.host}'"""
    ...
    def handle_code_prompt(self, code_prompt: str) -> str:
        command = f"""ssh -tt tcappendij@129.152.15.84 'cd data/volume_2/Greencode_framework/ && source venv/bin/activate && python3 deepseek-coder-6.7b-instruct/client.py --prompt "{code_prompt}" --port {self.port} --host {self.host}'"""
    ...
    def close(self) -> int:
        print("Close deepseek-coder-6.7b-instruct")
